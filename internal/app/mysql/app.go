...
func (a *App) Connect() error {
	...
	a.logger.With(slog.String("fn", fn), slog.String("driver", "mysql"))

	if err := a.db.Ping(); err != nil {
		a.logger.Warn("failed to ping database connection", sloglib.Error(err))
...
func (a *App) Close() error {
	...
	if err := a.db.Close(); err != nil {
		a.logger.Warn("failed to close database", sloglib.Error(err))
		return fmt.Errorf("%s: failed to close database connection: %w", fn, err)
	}

	a.logger.Info("successfully closed database")
	return nil
}

func New(logger *slog.Logger, connectionString string) (*App, error) {
	const fn = "app.mysql.app.New"
	logger.With(slog.String("fn", fn), slog.String("driver", "mysql"))

	db, err := sql.Open("mysql", connectionString)
	if err != nil {
		logger.Warn("failed to open database connection", sloglib.Error(err))
